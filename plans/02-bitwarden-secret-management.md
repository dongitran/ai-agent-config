...
Package automatically scans all MCP configs và finds:
```
🔍 Found required secrets:
- ${GITHUB_TOKEN}
- ${OPENAI_API_KEY}
- ${DATABASE_PASSWORD}
```
...
- ✅ If set → proceed to fetch secrets
...
⚠ DATABASE_PASSWORD (not found - will skip)
```
...
✓ Added 2 secrets to ~/.zshrc
ℹ️  Run 'source ~/.zshrc' or restart terminal to apply
```
...
🚀 Ready to use! Restart terminal or run: source ~/.zshrc
```

**Note**: Không cần separate `ai-agent install` command - `pull` đã làm mọi thứ!
...
# ✓ Added 3 secrets to ~/.zshrc
#
# ✅ Pull completed! Restart terminal or run: source ~/.zshrc
