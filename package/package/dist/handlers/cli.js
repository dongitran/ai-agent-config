...
import { lockSchema, syncSchema, statusSchema, listSchema, getSchema, generateSchema, createItemSchema, createFolderSchema, editItemSchema, editFolderSchema, deleteSchema, confirmSchema, createOrgCollectionSchema, editOrgCollectionSchema, editItemCollectionsSchema, moveSchema, deviceApprovalListSchema, deviceApprovalApproveSchema, deviceApprovalApproveAllSchema, deviceApprovalDenySchema, deviceApprovalDenyAllSchema, restoreSchema, createTextSendSchema, createFileSendSchema, listSendSchema, getSendSchema, editSendSchema, deleteSendSchema, removeSendPasswordSchema, createAttachmentSchema, } from '../schemas/cli.js';
...
export const handleList = withValidation(listSchema, async (validatedArgs) => {
    const { type, search, organizationid, url, folderid, collectionid, trash } = validatedArgs;
    const params = [type];
    if (search) {
...
export const handleGenerate = withValidation(generateSchema, async (validatedArgs) => {
    const params = [];
...
export const handleCreateItem = withValidation(createItemSchema, async (validatedArgs) => {
    const { name, type, notes, login, card, identity, secureNote, folderId } = validatedArgs;
    ...
    if (type === 1 && login) {
        ...
        if (login.totp !== undefined)
            loginData.totp = login.totp;
        if (login.uris !== undefined)
            loginData.uris = login.uris;
        item.login = loginData;
    }
    else if (type === 2 && secureNote) {
        // Secure Note type
        const secureNoteData = {};
        if (secureNote.type !== undefined)
            secureNoteData.type = secureNote.type;
        ...
        if (card.expMonth !== undefined)
            cardData.expMonth = card.expMonth;
        if (card.expYear !== undefined)
            cardData.expYear = card.expYear;
        ...
        if (identity.state !== undefined)
            identityData.state = identity.state;
        ...
        if (identity.ssn !== undefined)
            identityData.ssn = identity.ssn;
...
export const handleCreateFolder = withValidation(createFolderSchema, async (validatedArgs) => {
...
export const handleEditItem = withValidation(editItemSchema, async (validatedArgs) => {
    const { id, name, notes, login, card, identity, secureNote, folderId } = validatedArgs;
    ...
    try {
        ...
        const existingItem = JSON.parse(getResponse.output || '{}');
...
export const handleEditFolder = withValidation(editFolderSchema, async (validatedArgs) => {
...
export const handleDelete = withValidation(deleteSchema, async (validatedArgs) => {
...
export const handleConfirm = withValidation(confirmSchema, async (validatedArgs) => {
...
export const handleCreateOrgCollection = withValidation(createOrgCollectionSchema, async (validatedArgs) => {
...
export const handleEditOrgCollection = withValidation(editOrgCollectionSchema, async (validatedArgs) => {
    const { organizationId, collectionId, name, externalId, groups } = validatedArgs;
...
export const handleEditItemCollections = withValidation(editItemCollectionsSchema, async (validatedArgs) => {
    ...
    const encodedJson = Buffer.from(collectionIdsJson, 'utf8').toString('base64');
    // Build the command
    const response = await executeCliCommand('edit', [
        'item-collections',
        itemId,
 ...
 */
export const handleMove = withValidation(moveSchema, async ({ itemId, organizationId, collectionIds }) => {
    // Encode the collection IDs array as JSON
    const collectionIdsJson = JSON.stringify(collectionIds);
    const encodedJson = Buffer.from(collectionIdsJson, 'utf8').toString('base64');
    // Build the command
 ...
 * @returns {Promise<McpResponse>} Promise resolving to the formatted result.
 */
export const handleDeviceApprovalList = withValidation(deviceApprovalListSchema, async ({ organizationId }) => {
    // Build the command
 ...
 * @returns {Promise<McpResponse>} Promise resolving to the formatted result.
 */
export const handleDeviceApprovalApprove = withValidation(deviceApprovalApproveSchema, async ({ organizationId, requestId }) => {
    // Build the command
    const response = await executeCliCommand('device-approval', [
        'approve',
        '--organizationid',
 ...
 * @returns {Promise<McpResponse>} Promise resolving to the formatted result.
 */
export const handleDeviceApprovalApproveAll = withValidation(deviceApprovalApproveAllSchema, async ({ organizationId }) => {
    // Build the command
    const response = await executeCliCommand('device-approval', [
        'approve-all',
        '--organizationid',
 ...
 * @returns {Promise<McpResponse>} Promise resolving to the formatted result.
 */
export const handleDeviceApprovalDeny = withValidation(deviceApprovalDenySchema, async ({ organizationId, requestId }) => {
    // Build the command
    const response = await executeCliCommand('device-approval', [
        'deny',
        '--organizationid',
 ...
 * @returns {Promise<McpResponse>} Promise resolving to the formatted result.
 */
export const handleDeviceApprovalDenyAll = withValidation(deviceApprovalDenyAllSchema, async ({ organizationId }) => {
    // Build the command
    const response = await executeCliCommand('device-approval', [
        'deny-all',
        '--organizationid',
...
export const handleRestore = withValidation(restoreSchema, async (validatedArgs) => {
    const { object, id } = validatedArgs;
    const response = await executeCliCommand('restore', [object, id]);
    return toMcpFormat(response);
});
export const handleCreateTextSend = withValidation(createTextSendSchema, async (validatedArgs) => {
    const { name, text, hidden, notes, password, maxAccessCount, expirationDate, deletionDate, disabled, } = validatedArgs;
    ...
    if (maxAccessCount)
        send.maxAccessCount = maxAccessCount;
    if (expirationDate)
        send.expirationDate = expirationDate;
...
export const handleCreateFileSend = withValidation(createFileSendSchema, async (validatedArgs) => {
    const { name, filePath, notes, password, maxAccessCount, expirationDate, deletionDate, disabled, } = validatedArgs;
    // Build the Send object
    const send = {
        type: 1, // File Send
        name,
...
export const handleGetSend = withValidation(getSendSchema, async (validatedArgs) => {
    const { id } = validatedArgs;
    const response = await executeCliCommand('send', ['get', id]);
    return toMcpFormat(response);
});
export const handleEditSend = withValidation(editSendSchema, async (validatedArgs) => {
    const { id, name, notes, password, maxAccessCount, expirationDate, deletionDate, disabled, } = validatedArgs;
    ...
    try {
        ...
        const existingSend = JSON.parse(getResponse.output || '{}');
        // Update with new values
        if (name !== undefined)
            existingSend.name = name;
        if (notes !== undefined)
            existingSend.notes = notes;
        if (password !== undefined)
            existingSend.password = password;
        ...
        if (deletionDate !== undefined)
            existingSend.deletionDate = deletionDate;
        if (disabled !== undefined)
            existingSend.disabled = disabled;
...
export const handleDeleteSend = withValidation(deleteSendSchema, async (validatedArgs) => {
    const { id } = validatedArgs;
    const response = await executeCliCommand('send', ['delete', id]);
    return toMcpFormat(response);
});
export const handleRemoveSendPassword = withValidation(removeSendPasswordSchema, async (validatedArgs) => {
    const { id } = validatedArgs;
    const response = await executeCliCommand('send', ['remove-password', id]);
    return toMcpFormat(response);
});
export const handleCreateAttachment = withValidation(createAttachmentSchema, async (validatedArgs) => {
